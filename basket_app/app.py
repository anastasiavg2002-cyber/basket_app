...
if "total_category_mapping" not in st.session_state:
    st.session_state.total_category_mapping = None
...
def generate_project_mapping(
    uploaded_zip,
    total_file
):

    projects = get_projects_from_zip(
        uploaded_zip
    )
    ...
    # при полном совпадении названий
...
# ============================================================
# TOTAL SECOND COLUMN CONFIGURATION
# ============================================================

def generate_total_category_mapping(
    total_file
):
...
if projects_zip:
    ...
    if (
        ...
        st.session_state.projects_signature
        != projects_signature
...
if projects_zip and total_file:
    ...
    if (
        ...
        st.session_state.total_signature
        != total_signature

        or

        st.session_state.projects_signature
        != projects_zip.file_id
...
if total_file:
    ...
    if (
        ...
        st.session_state.total_category_mapping is None
        ...
        st.session_state.total_category_mapping = (
            generate_total_category_mapping(
                total_file
            )
        )

    st.subheader(
        "🔄 Total categories configuration"
    )

    st.info(
        "Можно изменить названия значений второго столбца total. "
        "Анализ будет выполнен уже по новым названиям."
    )
    ...
    st.session_state.total_category_mapping = (
        st.data_editor(
            ...
            key="total_category_mapping_editor"
...
if st.button("RUN"):
    ...
    if st.session_state.sheet_config is None:
        ...
        st.error(
            "Sheet configuration is missing"
        )
    ...
    if st.session_state.project_mapping is None:
        ...
        st.error(
            "Project mapping is missing"
        )

        st.stop()

    if st.session_state.total_category_mapping is None:

        st.error(
            "Total category mapping is missing"
        )
    ...
    st.session_state.total_category_mapping.to_excel(
