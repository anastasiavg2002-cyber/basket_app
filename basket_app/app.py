...
# CONFIGURATION OF SECOND COLUMN IN TOTAL

# ============================================================

def generate_total_category_mapping(total_file):
...
if (
    ...
    st.session_state.total_category_mapping = (
        generate_total_category_mapping(total_file)
    )

st.subheader("📄 Total categories configuration")

st.info(
    "Можно изменить названия категорий во втором столбце total.xlsx."
)

st.session_state.total_category_mapping = st.data_editor(
    st.session_state.total_category_mapping,
    use_container_width=True,
    hide_index=True,
...
if st.session_state.total_category_mapping is None:
    ...
    st.error("Total category configuration is missing")
    st.stop()
